...
class DataSyncServiceClass {
    ...
    * @returns true if synced successfully, false otherwise
     */
    async syncAttempt(attempt) {
        const patientId = await this.getPatientId();
        const payload = { ...attempt, patientId };
        try {
            const response = await fetchWithTimeout(`${getApiBaseUrl()}/api/data/attempts`, {
                method: 'POST',
    ...
    * @returns true if synced successfully, false otherwise
    ...
    async syncSession(session) {
        ...
        try {
            const response = await fetchWithTimeout(`${getApiBaseUrl()}/api/data/sessions`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
     ...
     */
    async fetchCloudSummary() {
        const patientId = await this.getPatientId();
        try {
            const response = await fetchWithTimeout(`${getApiBaseUrl()}/api/data/summary/${patientId}`, { method: 'GET' });
            if (response.ok)
     ...
     */
    async fetchCloudAttempts(activity, limit = 500) {
        const patientId = await this.getPatientId();
        ...
        try {
            const response = await fetchWithTimeout(`${getApiBaseUrl()}/api/data/attempts/${patientId}?${params}`, { method: 'GET' });
            if (response.ok) {
     ...
     */
    async fetchCloudSessions(limit = 500) {
        const patientId = await this.getPatientId();
        const params = new URLSearchParams();
        params.set('limit', limit.toString());
        try {
            const response = await fetchWithTimeout(`${getApiBaseUrl()}/api/data/sessions/${patientId}?${params}`, { method: 'GET' });
            if (response.ok) {
                const data = await response.json();
