...
// ============================================================================
export const DrawingCanvas = forwardRef(({ onStrokeEnd, strokeWidth = 14, canvasHeight, showGuideLines = true, }, ref) => {
    // ========================================================================
    // STATE MANAGEMENT
    // ========================================================================
     ...
     */
    const captureStrokeWidth = Math.max(ML_TARGET_STROKE_AT_28 * (canvasHeight / 28), strokeWidth * 1.2);
    // ========================================================================
    ...
    return (<View style={{ width: canvasHeight, height: canvasHeight }}>
            {/* ============================================================ */}
            {/* DISPLAY VIEW - What the user sees and interacts with */}
            {/* ============================================================ */}
            <View style={[styles.outerFrame, { width: canvasHeight, height: canvasHeight }]}>
                <View ref={canvasViewRef} style={styles.canvasContainer} {...panResponder.panHandlers} collapsable={false}>
                    {/* SVG layer for drawing */}
                    <Svg style={StyleSheet.absoluteFill} pointerEvents="none">
                        {/* Dyslexia-friendly warm peach background */}
                        <Rect x="0" y="0" width="100%" height="100%" fill={colors.peachLight}/>

                        {/* Guide lines - help with letter proportion */}
                        {showGuideLines && (<>
                                {/* Horizontal center line */}
                                <Line x1="0" y1="50%" x2="100%" y2="50%" stroke={colors.grey} strokeWidth="1.5" strokeDasharray="8,8"/>
                                {/* Vertical center line */}
                                <Line x1="50%" y1="0" x2="50%" y2="100%" stroke={colors.grey} strokeWidth="1.5" strokeDasharray="8,8"/>
                            </>)}

                        {/* Completed paths - display style */}
                        {paths.map((d, index) => (<Path key={index} d={d} stroke={colors.stroke} strokeWidth={strokeWidth} fill="none" strokeLinecap="round" strokeLinejoin="round"/>))}

                        {/* Current path being drawn */}
                        {currentPath && (<Path d={currentPath} stroke={colors.stroke} strokeWidth={strokeWidth} fill="none" strokeLinecap="round" strokeLinejoin="round"/>)}
                    </Svg>

                    {/* Placeholder shown when canvas is empty */}
                    {!hasDrawing && (<View style={styles.placeholderContainer}>
            ...
            {/* ============================================================ */}
            <View ref={captureViewRef} style={[
            styles.hiddenCaptureView,
            { width: canvasHeight, height: canvasHeight }
        ]} collapsable={false} pointerEvents="none">
                <Svg width={canvasHeight} height={canvasHeight} viewBox={`0 0 ${canvasHeight} ${canvasHeight}`}>
                    {/* Pure white background */}
                    <Rect x="0" y="0" width={canvasHeight} height={canvasHeight} fill="#FFFFFF"/>

                    {/* Completed paths - ML-optimized style */}
                    {paths.map((d, index) => (<Path key={`cap-${index}`} d={d} stroke="#000000" strokeWidth={captureStrokeWidth} fill="none" strokeLinecap="round" strokeLinejoin="round"/>))}

                    {/* Current path (included in case capture happens mid-stroke) */}
                    {currentPath && (<Path d={currentPath} stroke="#000000" strokeWidth={captureStrokeWidth} fill="none" strokeLinecap="round" strokeLinejoin="round"/>)}
                </Svg>
            </View>
        </View>);
});
