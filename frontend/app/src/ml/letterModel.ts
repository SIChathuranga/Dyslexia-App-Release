...
import {
    predictLetterFromImage,
    predictLetterDyslexiaFriendly,
    checkBackendHealth,
    getModelInfo,
} from '../api/letterApi';
 ...
 *   }, []);
 */
export const loadLetterModel = async (): Promise<void> => {
 ...
 */
export const isInferenceReady = (): boolean => {
...
/**
 * Basic Letter Prediction Result
 * Used for simple prediction without dyslexia-specific features
 */
...
/**
 * Dyslexia-Friendly Prediction Result
 * Includes similarity analysis and encouraging feedback
 */
export interface DyslexiaFriendlyResult {
    /** Predicted letter (e.g., "A") */
    letter: string;
    ...
    /** The letter the child was asked to draw */
    expectedLetter?: string;

    /** True if prediction exactly matches expected letter */
    isExactMatch: boolean;

    /** True if predicted letter is visually similar to expected */
    isSimilarLetter: boolean;

    /** List of letters similar to the expected letter */
    ...
    /** Feedback level: 'excellent' (exact), 'good' (similar), 'try_again' */
    feedbackLevel: 'excellent' | 'good' | 'try_again';
 ...
 */
export const predictLetterFromBase64 = async (
    base64Image: string
    ...
    try {
        ...
        return {
            ...
            topPredictions: (result as any).top_predictions || []
        };
    } catch (error) {
 ...
 *   }
 */
export const predictLetterDyslexiaFriendlyFromBase64 = async (
    base64Image: string,
    expectedLetter: string
): Promise<DyslexiaFriendlyResult> => {
    // Ensure backend is connected
    if (!backendConnected) {
        console.warn('Backend not connected, attempting to reconnect...');
        await loadLetterModel();
 ...
 *   }
 */
export const isLetterInPredictions = (target: string, prediction: LetterPrediction): boolean => {
 ...
 */
export const predictLetter = async (
    paths: string[]
