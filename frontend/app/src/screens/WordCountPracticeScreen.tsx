 ...
 */
const SIMILAR_DIGITS: { [key: string]: string[] } = {
    '0': ['6', 'O'],
    '1': ['7', 'I', 'L'],
    '2': ['Z'],
    '3': ['8'],
 ...
 */
export const WordCountPracticeScreen: React.FC = () => {
    // ========================================================================
    // HELPER FUNCTIONS
    // ========================================================================
     ...
     */
    const isAcceptableAnswer = (predicted: string, expected: string): boolean => {
        // Exact match
        if (predicted === expected) return true;

        // Check similar digits
        const similarToExpected = SIMILAR_DIGITS[expected] || [];
        if (similarToExpected.includes(predicted)) return true;
        ...
        try {
            ...
            if (isAcceptableAnswer(predictedDigit, expectedAnswer)) {
                ...
                await SessionTracker.recordAttempt(
                    'word_count', expectedAnswer, predictedDigit, true, `Word: ${currentWord}`
                );
            } else {
                ...
                await SessionTracker.recordAttempt(
                    'word_count', expectedAnswer, predictedDigit, false, `Word: ${currentWord}`
                );
            }
        } catch (error) {
     ...
     */
    const formatWordWithSpacing = (word: string) => {
    ...
    return (
        <SafeAreaView style={styles.container} edges={['bottom']}>
                    ...
                    <View style={[styles.speechBubble, { padding: bubblePadding }]}>
                        ...
                        <View style={styles.letterHint}>
                            {currentWord.split('').map((_, index) => (
                                <View key={index} style={[styles.letterDot, { width: dotSize, height: dotSize, borderRadius: Math.round(dotSize / 2) }]} />
                            ))}
                        </View>
                    </View>
            ...
            <FeedbackCard
                ...
                autoDismissMs={2000}  // 2 seconds for longer messages
            />
        </SafeAreaView>
    );
};
