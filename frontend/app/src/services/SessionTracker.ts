...
/** Activity types matching the 4 screens */
export type ActivityType = 'letter_practice' | 'math_practice' | 'word_count' | 'letter_hunt';
...
export interface AttemptRecord {
    ...
    timestamp: number;
    /** What was expected (target letter/number/word) */
    expected: string;
    /** What the ML model predicted */
    predicted: string;
    /** Was the attempt correct? */
    isCorrect: boolean;
    /** How many milliseconds between showing the challenge and checking */
    responseTimeMs: number;
    /** Number of retries for this specific challenge */
    retryCount: number;
    /** Additional context (e.g., word used, position, equation) */
    context?: string;
}

/** Session summary for a period of app usage */
...
export interface WeeklyProgress {
    ...
    totalCorrect: number;
    accuracy: number;
    activitiesPlayed: ActivityType[];
    avgResponseTimeMs: number;
    sessionsCount: number;
}

/** Activity stats for visualization */
export interface ActivityStats {
    ...
    accuracy: number;
    avgResponseTimeMs: number;
    mostConfused: { expected: string; predicted: string; count: number }[];
...
export interface ParentSuggestion {
    id: string;
    category: 'strength' | 'improvement' | 'practice' | 'encouragement';
    icon: string;
    title: string;
    message: string;
...
export interface PatientAnalysis {
    ...
    confusionMatrix: { expected: string; predicted: string; count: number; activity: ActivityType }[];
...
export const ACTIVITY_DISPLAY_NAMES: { [key in ActivityType]: string } = {
...
export const ACTIVITY_EMOJIS: { [key in ActivityType]: string } = {
    letter_practice: '✏️',
    math_practice: '🔢',
    word_count: '📝',
    letter_hunt: '🔍',
...
export const ACTIVITY_COLORS: { [key in ActivityType]: string } = {
...
class SessionTrackerService {
    ...
    private lastExpected: string | null = null;
     ...
     */
    startChallenge(expected: string): void {
    ...
    async getAttempts(): Promise<AttemptRecord[]> {
        try {
            return await DataSyncService.fetchCloudAttempts(undefined, 5000) as AttemptRecord[];
        } catch {
            return [];
        }
    }
    ...
    async getSessions(): Promise<SessionSummary[]> {
        try {
            return await DataSyncService.fetchCloudSessions(1000) as SessionSummary[];
        } catch {
            return [];
        }
    }
     ...
     */
    async generateAnalysis(): Promise<PatientAnalysis> {
     ...
     */
    private calculateActivityStats(attempts: AttemptRecord[]): ActivityStats[] {
        const activities: ActivityType[] = ['letter_practice', 'math_practice', 'word_count', 'letter_hunt'];
     ...
     */
    private calculateWeeklyProgress(attempts: AttemptRecord[]): WeeklyProgress[] {
        ...
        for (let i = 7; i >= 0; i--) {
            ...
            const playedActivities = [...new Set(weekAttempts.map(a => a.activity))] as ActivityType[];
     ...
     */
    private calculateConfusionMatrix(attempts: AttemptRecord[]) {
        const confusionMap = new Map<string, { expected: string; predicted: string; count: number; activity: ActivityType }>();
     ...
     */
    private calculateTrend(attempts: AttemptRecord[]): 'improving' | 'stable' | 'declining' {
     ...
     */
    private calculateStreaks(attempts: AttemptRecord[]): { currentStreak: number; longestStreak: number } {
     ...
     */
    private calculateActivityStreak(attempts: AttemptRecord[]): number {
